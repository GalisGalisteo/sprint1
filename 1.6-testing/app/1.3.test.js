...
test('getEmployee: should return the employee object when ID is valid', async () => {
...
test('getEmployee: if idToFind does not exists should print an error', async () => {
    await expect(getEmployee(5)).rejects.toThrowError("The id 5 doesn't exist");
});

test('getEmployee: if idToFind is a non asigned variable should throw an error', async () => {
    await expect(getEmployee()).rejects.toThrowError('The id is not a number or is undefined');
});

test('getEmployee: if idToFind is a string should throw an error', async () => {
...
test('getSalary: if the employee does not exists should print an error', async () => {
    const employee = {
        id: 6,
        name: 'Yao Ming'
    }
...
test('getSalary: if the employee is a string should print an error', async () => {
    const employee = 'hola'
    await expect(getSalary(employee)).rejects.toThrowError('You need to insert de data of an Employee');
});

test('getSalary: if the employee is a number should print an error', async () => {
    const employee = 1;
    await expect(getSalary(employee)).rejects.toThrowError('You need to insert de data of an Employee');
});


test('getSalary: if the employee name exists but has a non-existing ID', async () => {
    const employee = {
        id: 999,
        name: 'Linux Torvalds'
    };
...
test('getSalary: if the employee name does not exists but has an existing ID', async () => {
    const employee = {
        id: 1,
        name: 'Yao Ming'
    };
...
test('getSalary: if the employee name exists but has a missing id property', async () => {
    const employee = {
        name: 'Linux Torvalds'
    };
    await expect(getSalary(employee)).rejects.toThrowError("The employee doesn't exist");
...
test('getSalary: if the employee exists but has an incorrect id data type', async () => {
    const employee = {
        id: '1',
        name: 'Linux Torvalds'
    };
...
test('getSalary: if the employee exists but has an additional property', async () => {
    const employee = {
        id: 1,
        name: 'Linux Torvalds',
        age: 30
